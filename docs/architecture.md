...
SSE 接口复用 `AgentService.chat_stream()`，因此与 CLI 使用同一套事件归一化逻辑。API 会过滤 `internal_state`，不会把 DeepAgents/LangGraph 的内部状态同步事件暴露给前端。API 启动时加载完整 Agent registry，请求体不支持切换 Agent；旧 `/chat` 与 `/chat/stream` 使用启动参数中的默认 Agent，新 `/agents/{agent_name}/...` 路由通过 URL 选择 Agent。聊天请求通过 Body 字段 `user_id` 区分用户作用域，未传时使用默认用户 `default`。跨域访问通过 FastAPI `CORSMiddleware` 实现，只有 `agents.yaml` 的 `settings.api.cors.origins` 配置非空后才启用 CORS。流式输出默认为 data-only（不含 `event:` 行），并且默认不重复发送尾部 `assistant_message`；可分别通过 `settings.api.stream.include_event_name` 与 `settings.api.stream.include_assistant_message` 调整行为。
...
主要配置字段包括：`app_name`、`debug`、`default_user_id`，以及分组对象 `storage`（`backend`、`db_path`、`postgres_dsn`、`sessions_dir`）与 `api`（`host`、`port`、`reload`、`cors.*`、`stream.*`）。
...
对话历史库是应用级共享数据库，默认使用 SQLite，位于 `./data/agent.sqlite3`；也可通过 `agents.yaml` 的 `settings.storage.backend: postgres` 与 `settings.storage.postgres_dsn` 切换到 PostgreSQL。
