...
SSE 接口复用 `AgentService.chat_stream()`，因此与 CLI 使用同一套事件归一化逻辑。API 会过滤 `internal_state`，不会把 DeepAgents/LangGraph 的内部状态同步事件暴露给前端。API 启动时加载完整 Agent registry，请求体不支持切换 Agent；旧 `/chat` 与 `/chat/stream` 使用启动参数中的默认 Agent，新 `/agents/{agent_name}/...` 路由通过 URL 选择 Agent。聊天请求通过 Body 字段 `user_id` 区分用户作用域，未传时使用默认用户 `default`。跨域访问通过 FastAPI `CORSMiddleware` 实现，只有设置 `AGENT_CORS_ORIGINS` 后才启用 CORS。
...
- 会话文件：`AGENT_SESSIONS_DIR`、`AGENT_DEFAULT_USER_ID`
...
`AGENT_DEFAULT_USER_ID` 默认是 `default`。这是用户隔离标识，不是认证或授权机制。CLI 可用 `--user-id` 指定启动用户，交互中可用 `/user` 查看当前用户、`/user <id>` 切换后续消息的用户作用域。API 聊天请求可在 Body 中传入 `user_id`。
